 ...
 * Game-Data File(s) are saved in a unique minimum head datastructures and can't
 * be loaded without proper decoding.
 * @version 1.0
 * @author Fabian001
 */
public final class FileCoder {
     ...
     * @throws GameDataFileSyntaxException The passed argument does not
     *                                     start with "sources: [.....]".
     */
    static ArrayList<String> extractPaths(final String input)
        ...
        if (!matcher2.find()) {
            throw new GameDataFileSyntaxException(
                    "An Error encountered with the GameDataFile Syntax:"
                    +        " No 'sources:' found"
            );
        }
        ...
        if (pathsString.contains(",")) {
            System.out.println(
                    "Syntax-Warning-ImageSource: "
                    +        "Comma found where a semicolon should be!"
            );
     ...
     * @throws GameDataFileSyntaxException If a group couldn't be found with its
     *                                     groupID.
     * @TODO-Fabian Custom-Assets werden noch nicht geladen!!!
     */
    public static String encodeAssets(final HashMap<Integer, Group> groups,
            final HashMap<Integer, Entity> entity)
            throws GameDataFileSyntaxException {
        ...
        if (main == null) {
            throw new GameDataFileSyntaxException(
                    "No main-group detected!!! - Encoding failed"
            );
        }
        builder.append("[main;");
        ...
        for (Integer currentChildGroup : main.getChildGroupIDs()) {
            ...
            if (currentGroup == null) {
                throw new GameDataFileSyntaxException("No group found with ID: "
                        + currentChildGroup
                        + "!!! - Encoding failed");
            }
            ...
            builder.append("    ");
            builder.append(getAssetViaGroup(currentGroup,
                    entity,
                    groups,
                    spaces));
            builder.append(System.lineSeparator());
    ...
    /**
     * Recursive function for loading assets from the currentGroup
     * and repeat the process if the group has subgroups nested.
     ...
     * @return Correct GameDataSyntax
     */
    public static String getAssetViaGroup(final Group currentGroup,
            final HashMap<Integer, Entity> entity,
            final HashMap<Integer, Group> groups,
            final StringBuilder spaces) {
        StringBuilder builder = new StringBuilder();
        ...
        for (Integer currentGroupID : currentGroup.getChildGroupIDs()) {
            ...
            System.out.println("Working: Subgroup: "
                    + currentChildGroup.getGroupName()
                    + "| spaces: " + spaces.length());

            builder.append(System.lineSeparator());
            builder.append(spaces);
            builder.append(getAssetViaGroup(currentChildGroup,
                    entity,
                    groups,
                    spiral.append("    ")));
            builder.append(System.lineSeparator());
     ...
     */
    public static String encodeImageIcons(
            final HashMap<Integer, ImageIcon> icons) {
        return "sources:[" + icons.values()
                .stream()
                .map(imageIcon -> {
                    String path = imageIcon.getDescription();
                    int textureIndex = path.indexOf("\\textures\\");
                    // If "\textures\" is not found, return the original path
                    if (textureIndex != -1) {
                        path = path
                                .substring(textureIndex + "\\textures\\"
                                        .length())
                                .replace("\\", "/");
                        if (!path.startsWith("/")) {
                            path = "/" + path;
     ...
     * @return Encoded image icons.
     */
    public static String encodeImageIcons(final ImageIcon[] icons) {
        ...
        for (int i = 0; i < icons.length; i++) {
            ...
            if (textureIndex != -1) {
                path = "/" + path
                        .substring(textureIndex + "\\textures\\".length())
                        .replace("\\", "/");
            }
            if (i != 0) {
                result.append(";");
